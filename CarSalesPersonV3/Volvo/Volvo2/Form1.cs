...
namespace Volvo2
{
    public partial class frmVolvoProgram : Form
    {
        ...
        //declaration for reading and connecting to database
        OleDbConnection dataConnect;
        ...
        public frmVolvoProgram()
        {
            ...
            //this.logInTableAdapter1.Fill(this.database1DataSet.logIn);
        ...
        private void btnNext1_Click(object sender, System.EventArgs e)
        {
            if (validateForm() == true)
            {
                ...
                order1.Date = dateTimePanel1.Value.ToString();
              //  MessageBox.Show(order1.Date);
            }
        }
        ...
        */
        ...
        {
            DialogResult res = new DialogResult();
            res = MessageBox.Show("Would you like to sumbit all information to the database?", "Submit", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (res == DialogResult.Yes)
            {
                panelList[4].Visible = false;
                panelList[5].Visible = true;
                customerNameTextBox.Text = order1.Name;
                customerAddressTextBox.Text = order1.Address;
                zipTextBox.Text = order1.zipCoder;
                phoneNumberTextBox.Text = order1.PhoneNumber;
                stateTextBox.Text = order1.stateName;
                dateTimePickerInfo.Text = order1.Date;

                //clear list box
                customerListBox.Items.Clear();

                //add each customerList in list to my list box
                foreach (string name in customerList)
                {
                    customerListBox.Items.Add(name);
                }

               // iDTextBox.Text = (numCustomers + 1).ToString();
            }
        ...
        private void btnAddToList_Click(object sender, EventArgs e)
        {
            ...
            //add each artist in list to my list box
            foreach (string name in customerList)
            {
                customerListBox.Items.Add(name);
        ...
        {
            DialogResult res = new DialogResult();
            res = MessageBox.Show("Are you sure you want to Exit?", "Submit", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (res == DialogResult.Yes)
                this.Close();
        }
        ...
        {
            order1.Name = customerNameTextBox.Text;
            order1.Address = customerAddressTextBox.Text;
            order1.zipCoder = zipTextBox.Text;
            order1.PhoneNumber = phoneNumberTextBox.Text;
            order1.stateName = stateTextBox.Text;
            order1.Date = dateTimePickerInfo.Value.ToString();
            connection(2);
        }
        ...
        {
            /*
           // string sqlCommand = "INSERT INTO logIn ([userName] , [password]) VALUES ('Fiona' , 'Leonia');";
            
            string sqlCommand2 = "INSERT INTO order ([AccountNum],[Name], [Address] , [Zip], [Pnumber], [Model], [carPrice], [titleP]" +
                ", [tax], [fullPrice], [monthlyFin], [APR], [priceMonthly], [cashRate], [cashPrice], [date]"+
                " VALUES ('" + order1.AccountNumber.ToString() + "','"+ order1.Name +"','" + order1.Address + "' ,'" + order1.zipCoder + "', '"+ order1.PhoneNumber + ""+
                "', '"+ order1.Model  +"', '" + order1.priceCar.ToString() + "', '" + order1.getTitlePrice().ToString("C") + "'"+
                ", '" + order1.getTaxRate().ToString("P1") + "', '" + order1.fullP.ToString("C") + "' , '" +
                order1.monthFin + "', '"+ order1.getFinRate().ToString("C") 
                +"', '"+ order1.MonthlyPrice.ToString("C")+ "', '" +order1.getCashRate().ToString("C") 
                + "', '" +order1.cashP.ToString("C") + "' , '" + order1.Date + "');";

            // MessageBox.Show(sqlCommand);
            string sqlCommand3 = "INSERT INTO package ([Name],[Price]) VALUES "+
                " ('" + package1.Name + "', '" + package1.Price.ToString("C") + "');";

            string sqlCommand4 = "INSERT INTO paint ([Color],[Price],[Metallic]) VALUES ('" + paint1.Color + "', '" + paint1.paintP.ToString() + 
                "', '" + paint1.Metallic+ "')";

            string sqlCommand5 = "INSERT INTO tradeIn ([Make],[Model], [Mileage], [Year], [carDest] , [carCon], [Msrp], [TradeValue]) VALUES ('"+ trade1.Make +
                "', '" + trade1.Model + "', '" + trade1.Mileage + "', '" + trade1.Year + "', '" + trade1.carDest + 
                "', '" + trade1.carCon + "', '" + trade1.Msrp.ToString("C") + "', '" + trade1.TradeValue.ToString("C") + "')";

            string[] commands = { sqlCommand2, sqlCommand3, sqlCommand4, sqlCommand5 };
            string con = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=|DataDirectory|\\Database1.mdb";
            //string test1 = "SELECT * FROM logIn";
            try
            {
                ...
                dataConnect = new OleDbConnection(con);
                dataConnect.Open();
                for (int i = 0; i < length; i++)
                {
                    dataCommand = new OleDbCommand();
                    dataCommand.CommandText = commands[i];
                    dataCommand.Connection = dataConnect;
            ...
            //execute the test1
            //dataCommand = new OleDbCommand();
            //dataCommand.CommandText = test1;
            //dataCommand.Connection = dataConnect;
            //dataReader = dataCommand.ExecuteReader();
            //load everything insdie the selected table
           // loadData(0);
        }

        //loading items into the table
        public void loadData(double num)
        {
            string con = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=|DataDirectory|\\Database1.mdb";
            string test1, sqlC = "";
            //load order table
            if (num == 1)
                sqlC = "SELECT * FROM order";
            //load model table
            else if (num == 2)
                sqlC = "SELECT * FROM model";
            else if (num == 3)
                sqlC = "SELECT * FROM package";
            else if (num == 4)
                sqlC = "SELECT * FROM paint";
            else if (num == 5)
                sqlC = "SELECT * FROM tradeIn";
            else if (num == 0)
                sqlC = "SELECT * FROM logIn";
            ...
            try
            {
                ...
                conn.Open();
                OleDbCommand oconn = new OleDbCommand(test1, conn);
        ...
        public void connection(double num)
        {
            string con = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=|DataDirectory|\\Database1.mdb";
            dataConnect = new OleDbConnection(con);
            dataConnect.Open();
            ...
            {
                dataCommand = new OleDbCommand();
                string sqlCommand = "SELECT customerName FROM Customer";
                ...
                {
                    string tempCustomer;
                    tempCustomer = dataReader["customerName"].ToString();
            ...
            {
                dataCommand = new OleDbCommand();
                string sqlCommand = "INSERT INTO Customer ([CustomerName], [CustomerAddress]," +
                    "[Zip], [PhoneNumber], [State], [Time])  VALUES ('" + order1.Name + "'"
                    + " ,'" + order1.Address + "', '" + order1.zipCoder + "', '" +
                    order1.PhoneNumber + "' ,'" + order1.stateName + "', '" +
                    order1.Date + "')";
                dataCommand.CommandText = sqlCommand;
                dataCommand.Connection = dataConnect;
                try
